...
export class IndirizzoService {
  ...
  private apiUrl: string = environment.apiURL + 'v4/indirizzo';
  ...
  getCAP(params): Observable<any> {
    this.headers = this.headers.set('Authorization', globals.userid);
    return this.httpClient.get<any>(this.apiUrl + `/findCap/${params}`, { headers: this.headers });
  }

  addIndirizzo(params): Observable<any> {
    this.headers = this.headers.set('Authorization', globals.userid);
  ...
  putIndirizzo(params): Observable<any> {
    this.headers = this.headers.set('Authorization', globals.userid);
  ...
  deleteIndirizzo(params): Observable<any> {
    this.headers = this.headers.set('Authorization', globals.userid);
